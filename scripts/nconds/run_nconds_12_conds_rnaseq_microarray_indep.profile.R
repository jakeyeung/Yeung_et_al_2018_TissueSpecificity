...
FitGetBIC <- function(dat, des.mat, bic.only = TRUE){
  return(BIC(lm(exprs ~ 0 + des.mat, data = dat)))
}
...
out.sub <- lapply(tiss.combos, function(tiss.combo){
  des.mat.sub <- SubsetFullDesignMatrix(des.mat.full, tiss.combo)
  fit.sub <- dat.sub %>% 
#     do(mod = FitRhythmicDesMat(., des.mat.sub)) %>%
    do(mod = FitGetBIC(., des.mat.sub)) %>%
    mutate(rhyth.tiss = paste0(tiss.combo, collapse = ","))
#   lm(formula = exprs ~ 0 + des.mat.sub, dat.sub)
})
