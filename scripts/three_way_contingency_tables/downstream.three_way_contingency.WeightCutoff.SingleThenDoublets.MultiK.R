...
RankedSingleMotifEnrichment <- function(N.mat.all, jgenes){
  N.mat.long <- melt(subset(N.mat.all, gene %in% jgenes), id.vars = c("gene", "peak", "model"), variable.name = "motif")
...
# Do singletons across phases ---------------------------------------------

K <- 300
# take subset of genes by phase, perform enrichment

jgenes <- unique(as.character(N.mat.all$gene))

fits.sing <- lapply(seq(100, 600, by = 50), function(K){
  print(K)
  ...
  load(inf, v=F)  # load also Nmat and Nmatfreqs
  fits.sing <- RankedSingleMotifEnrichment(N.mat.all, jgenes)
  fits.sing$K <- K
  return(fits.sing)
})

fits.sing <- bind_rows(fits.sing)

fits.sing.sum <- fits.sing %>%
  group_by(motif) %>%
...
ggplot(subset(fits.sing, motif %in% c("RORA", "HOX.A4.D4", "EVI1", "CEBPA.B_DDIT3")), aes(x = K, y = -log10(pval), colour = motif)) + geom_line() 
ggplot(subset(fits.sing, motif %in% fits.sing.sum$motif), aes(x = K, y = -log10(pval), colour = motif)) + geom_line() + theme(legend.position = "none")
...
# inf <- paste0(inf <- paste0("/home/yeung/projects/tissue-specificity/Robjs/three_way_cooccurence/three.way.cooccurrence.bugfixed.nmodels.3.K.300.weight.0.MergePeaks.FALSE.nullmodel.JI.flatampmax.", flatampmax, ".withNmatallNmatfreqs.RemoveZeroCounts.Robj"))
load(inf, v=T)
