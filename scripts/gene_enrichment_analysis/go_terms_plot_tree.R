...
fits.long.filt <- subset(fits.long.filt, method == jmethod)
fg.genes <- as.character(subset(fits.long.filt, model == jmod)$gene)
bg.genes <- as.character(subset(fits.long.filt)$gene)
sym2entrez <- CreateSym2Entrez()
fg.genes.entrez <- ConvertSym2Entrez(fg.genes, sym2entrez)
bg.genes.entrez <- ConvertSym2Entrez(bg.genes, sym2entrez)

data(exampledata)

jorgo <- "org.Mm.eg.db"
sigTerm <- GOFunction(fg.genes.entrez, bg.genes.entrez, organism=jorgo, ontology="BP", fdrmethod="BY", fdrth=0.05, ppth=0.05, pcth=0.05, poth=0.05, peth=0.05, bmpSize=2000, filename="sigTerm")

organism=jorgo; ontology="BP"; fdrmethod="BY"; 
fdrth=0.05; ppth=0.05; pcth=0.05; poth=0.05; 
peth=0.0; bmpSize=2000; filename="sigTerm"
...
annRef <- generalAnn[generalAnn[,1] %in% bg.genes.entrez,]
annInterest <- generalAnn[generalAnn[,1] %in% fg.genes.entrez,]

## Calculating the statistically significant terms for fg.genes.entrez
...
# take top 10 hits 
pvalcutoff <- 1
top.go.terms <- as.character(subset(sigTerm, pvalue < pvalcutoff)$goid)
...
plot(sigDAG, attrs = graphAttrs, nodeAttrs = nodeAttrs, edgeAttrs = edgeAttrs, main = "test2")
