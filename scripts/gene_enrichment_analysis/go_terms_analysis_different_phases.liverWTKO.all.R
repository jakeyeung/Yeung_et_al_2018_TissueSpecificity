...
enrichment <- mclapply(tstarts, function(tstart){
  ...
  enrichment <- GetGOEnrichment(genes.bg, genes, fdr.cutoff = 1, ontology = jonto, show.top.n = Inf, filter.GO.terms = FALSE)  # take all
...
# save to output
fout <- paste0("/home/yeung/projects/tissue-specificity/Robjs/GO_analysis/model", jmod.long, ".all.Robj")
