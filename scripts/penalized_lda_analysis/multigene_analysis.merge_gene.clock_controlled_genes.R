...
# Take all peaks assigned to the gene by tissue ---------------------------
...
S.sub.liverpeaks <- S.sub %>%
  group_by(peak) %>%
  filter(min(zscore[tiss.i]) > jcutoff)  # do min in case tiss.i is multi tissue
...
S.sub.nonliverpeaks <- S.sub %>%
  group_by(peak) %>%
  filter(max(zscore[others.i]) > jcutoff)
...
# Analyze genes with ROR elements -----------------------------------------


# what other motifs are near RORA?
# make long

# rora.hits <- subset(mat.fg, RORA.p2 > 0, select = c(gene, RORA.p2))
rora.hits <- subset(mat.bg, RORA.p2 > 0, select = c(gene, RORA.p2))
# df.sub <- melt(mat.fg, variable.name = "motif", id.vars = "gene", value.name = "sitecounts")
df.sub <- melt(mat.bg, variable.name = "motif", id.vars = "gene", value.name = "sitecounts")
df.sub <- df.sub %>%
  group_by(gene) %>%
  filter(sitecounts > 0) %>%
  arrange(sitecounts) %>%
...
# jgene <- "Dtd2"
jgene <- sample(x = rora.hits$gene, size = 1)
# ggplot(subset(df.sub, gene == jgene & motif %in% tissue.motifs), aes(x = indx, y = sitecounts, label = motif)) + geom_text() + ggtitle(jgene)
ggplot(subset(df.sub, gene == jgene & motif %in% liv.motifs), aes(x = indx, y = sitecounts, label = motif)) + geom_text() + ggtitle(jgene)

# How many ROR hits have a tissue-specific motif nearby? ------------------

liv.motifs <- c("HNF1A", "FOXA2", "HNF4A_NR2F1.2", "CUX2", "CEBPA.B_DDIT3", "RXRG_dimer", "ONECUT1.2")
df.sub.tiss <- subset(df.sub, gene %in% rora.hits$gene & motif %in% tissue.motifs)  # 25/26 is this specific to ROR?

# Compare signal between Liver rhyth and background -----------------------
