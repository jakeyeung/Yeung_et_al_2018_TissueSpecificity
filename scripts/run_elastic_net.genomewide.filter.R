...
load("/home/yeung/projects/tissue-specificity/results/fits.Robj")  # fit.list from find.oscillating.genes.R to filter rhythmic genes

conds <- c('Liver')
for (cond in conds){
  y <- read.table(paste0("y_input_elastic_net/genome_wide//genome_wide.", cond, ".elasticinput"))
  genes <- rownames(y)
  genes.rhythmicl <- sapply(genes, function(g){
    pval <- fit.list[[cond]][[g]]$pval
    if (is.nan(pval)){
      return(FALSE)
    }
    if (pval < cutoff){
  ...
  genes.filter <- genes[which(genes.rhythmicl == TRUE)]
  y <- y[genes.filter, ]
  head(rownames(x))
  head(rownames(y))
  ...
  print(cond)
  start.time <- Sys.time()
  RunElasticNet(y, x, file.path(output.dir, paste0("corrected_sitecounts_", cond, cutoff)), standardize = TRUE, alpha = alpha)
