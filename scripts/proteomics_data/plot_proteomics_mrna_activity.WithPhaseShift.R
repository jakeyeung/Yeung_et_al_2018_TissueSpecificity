...
for (jwt.prot in c("Cry")){
  ...
  for (genes in gene.lst){
    ...
    # jtest.singleday <- PlotmRNAActivityProtein(dat.long, act.shift, prot.long = prot.long, gene.dat = gene.dat,  gene.act = gene.act, gene.prot = gene.prot, jtiss = "Liver", dotsize = 3, themesize = 22, wt.prot = jwt.prot, line.for.protein = TRUE, act.in.sine.cos = FALSE, single.day = FALSE) + theme(strip.text = element_blank())
    # print(jtest + ggtitle(gene.dat))
    # print(jtest.both + ggtitle(gene.dat) + theme(strip.text = element_blank()))
    print(jtest.singleday)
    print(jtest.singleday.ampphase)
  }
  dev.off()
}
