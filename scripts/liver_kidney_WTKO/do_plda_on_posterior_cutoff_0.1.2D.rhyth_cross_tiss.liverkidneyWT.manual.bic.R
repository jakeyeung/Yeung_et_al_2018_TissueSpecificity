...
PlotPolarHistogram(subset(liv.rhyth), countstring = "Count", phase.cname = phase.cname)
...
jsub.plot$motif.uniq <- sapply(as.character(jsub.plot$motif), function(m) strsplit(m, ";")[[1]][[1]])
maxcutoff <- 0.5
jsub.plot.cutoff <- subset(jsub.plot, max.motif.count > maxcutoff)
n.genes <- length(unique(jsub.plot.cutoff$gene))
genes.lst <- unique(jsub.plot.cutoff$gene)

plot(density(unique(jsub.plot.cutoff$phase)), main = paste(jgrep, "ngenes:", n.genes), ylim=c(0, 0.15))
lines(density(unique(jsub.plot$phase)), type="l", lty=2, lwd=2,)

# do statistical test
phase.motif <- unique(jsub.plot.cutoff$phase)
phase.all <- unique(jsub.plot$phase)
ks.test(phase.motif, phase.all)
# qqplot(phase.motif, phase.all)

# plot genes
# print(genes.lst)

# ggplot(subset(jsub.gene, label == "Liv rhyth" & max.motif.count > 0.1), aes(x = phase)) +  ggtitle(paste(jmotifs, collapse=",")) + geom_histogram() + xlim(c(0, 24))
amp.max <- ceiling(max(jsub.plot$amp))

jsub.plot.sum <- jsub.plot %>%
  group_by(motif.uniq, gene) %>%
  summarise(max.motif.count = max(max.motif.count),
            amp = unique(amp),
            phase = unique(phase))

ggplot(jsub.plot.sum, aes(alpha = max.motif.count, x = amp, y = phase)) + coord_polar(theta = "y") + geom_point() + ggtitle(jgrep) +
...
# set cutoff
ggplot(subset(jsub.plot, max.motif.count > maxcutoff), aes(alpha = max.motif.count, x = amp, y = phase)) + coord_polar(theta = "y") + geom_point() + ggtitle(jgrep) +
  scale_y_continuous(limits = c(0, 24), breaks = seq(6, 24, 6)) +
  scale_x_continuous(limits = c(0, amp.max), breaks = seq(0, amp.max, length.out = 2)) +
  theme_bw() +
  # geom_hline(yintercept = seq(0, 5, by = 1), colour = "grey90", size = 0.2) +
...
plot(density(unique(subset(jsub.plot, max.motif.count > 0.5)$phase)), main = jgrep)
...
PlotGeneTissuesWTKO(subset(dat.long, gene == jgene & tissue == "Liver_SV129"))
PlotGeneAcrossTissues(subset(dat.long, gene == jgene & tissue == "Liver_SV129"))
