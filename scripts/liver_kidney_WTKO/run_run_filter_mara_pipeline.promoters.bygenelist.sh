...
outmain="/home/yeung/projects/tissue-specificity/results/MARA"
	...
	for dist in 10000 20000 40000; do
		for method in "BIC" "g=1001"; do
			for model in "Liver_SV129"; do
				...
				outdir=$outmain/atger_kidney.dist_"$dist".cutoff_"$cutoff".model_"$model".method_"$method".cross_"$cross".tspeaks_"$tspeaks"
				...
				Nmat=$Nmain/sitecounts_enhancers.model_"$model".method_"$method".dist_"$dist".cutoff_"$cutoff".cross_"$cross".tspeaks_"$tspeaks".mat
