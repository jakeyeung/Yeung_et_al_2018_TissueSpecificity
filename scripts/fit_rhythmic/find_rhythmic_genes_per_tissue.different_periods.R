...
dat.fit <- dat.long %>%
  ...
  split(.$tissue) %>%
  mclapply(., function(jdf) do(.data = jdf, FitRhythmic(., T.period = jperiod)), mc.cores = 12) %>%  # can change to 12 or 24
  do.call(rbind, .)
print(Sys.time() - start)

save(dat.fit, file = paste0("Robjs/dat.fit.period.", jperiod, ".Robj"))
